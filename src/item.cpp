...
void item::enchantment_info(std::vector<iteminfo>& info, const iteminfo_query* parts, int batch) const {
    ...
    if (relic_data) {
        ...
        double item_damage_acid = 0.0;
        double item_damage_heat = 0.0;
        double item_damage_cold = 0.0;
        ...
        double skill_rust_resist_mult =1.0;
        ...
        double item_damage_acid_mult = 1.0;
        double item_damage_heat_mult = 1.0;
        double item_damage_cold_mult = 1.0;
        ...
        for (enchant_cache& e : relic_data->get_proc_enchantments()) {
            ...
            skill_rust_resist += e.get_value_add(enchant_vals::mod::SKILL_RUST_RESIST);
            ...
            item_damage_acid += e.get_value_add(enchant_vals::mod::ITEM_DAMAGE_ACID);
            item_damage_heat += e.get_value_add(enchant_vals::mod::ITEM_DAMAGE_HEAT);
            item_damage_cold += e.get_value_add(enchant_vals::mod::ITEM_DAMAGE_COLD);
            ...
            skill_rust_resist_mult += e.get_value_multiply(enchant_vals::mod::SKILL_RUST_RESIST);
            ...
            item_damage_acid_mult += e.get_value_multiply(enchant_vals::mod::ITEM_DAMAGE_ACID);
            item_damage_heat_mult += e.get_value_multiply(enchant_vals::mod::ITEM_DAMAGE_HEAT);
            item_damage_cold_mult += e.get_value_multiply(enchant_vals::mod::ITEM_DAMAGE_COLD);
        ...
        for (enchantment& e : relic_data->get_defined_enchantments()) {
            ...
            skill_rust_resist += e.get_value_add(enchant_vals::mod::SKILL_RUST_RESIST, player);
            ...
            skill_rust_resist_mult += e.get_value_multiply(enchant_vals::mod::SKILL_RUST_RESIST, player);
        ...
        if (resonance != 0.0 || pain != 0.0 || speed != 0.0 || base_move_cost != 0.0 || attack_speed != 0.0
            ...
            if (climate_control_heat_mult != 1.0) {
                if (need_space) {
                    base_str += string_format("   <color_c_yellow>x %.2f</color>", climate_control_heat_mult);
                }
                else {
                    base_str += string_format("<color_c_yellow>x %.2f</color>", climate_control_heat_mult);
                }
            ...
            if (climate_control_chill_mult != 1.0) {
                if (need_space) {
                    base_str += string_format("   <color_c_yellow>x %.2f</color>", climate_control_chill_mult);
                }
                else {
                    base_str += string_format("<color_c_yellow>x %.2f</color>", climate_control_chill_mult);
                }
            ...
            base_str = "* 技能遗忘抗性：";
            need_space = false;
            if (skill_rust_resist != 0.0) {
                base_str += string_format("<color_c_yellow>%d</color>", static_cast<int>(skill_rust_resist));
                need_space = true;
            }
            if (skill_rust_resist_mult != 1.0) {
                if (need_space) {
                    base_str += string_format("   <color_c_yellow>x %.2f</color>", skill_rust_resist_mult);
                }
                else {
                    base_str += string_format("<color_c_yellow>x %.2f</color>", skill_rust_resist_mult);
                }
            ...
            base_str = "* 脚步声：";
            need_space = false;
            if (footstep_noise != 0.0) {
                base_str += string_format("<color_c_yellow>%d</color>", static_cast<int>(footstep_noise));
                need_space = true;
            }
            if (footstep_noise_mult != 1.0) {
                if (need_space) {
                    base_str += string_format("   <color_c_yellow>x %.2f</color>", footstep_noise_mult);
                }
                else {
                    base_str += string_format("<color_c_yellow>x %.2f</color>", footstep_noise_mult);
                }
            ...
            need_space = false;
            if (shout_noise != 0.0) {
                base_str += string_format("<color_c_yellow>%d</color>", static_cast<int>(shout_noise));
                need_space = true;
            }
            if (shout_noise_mult != 1.0) {
                if (need_space) {
                    base_str += string_format("   <color_c_yellow>x %.2f</color>", shout_noise_mult);
                }
                else {
                    base_str += string_format("<color_c_yellow>x %.2f</color>", shout_noise_mult);
                }
            ...
            need_space = false;
            if (block_nums != 0.0) {
                base_str += string_format("<color_c_yellow>%d</color>", static_cast<int>(block_nums));
                need_space = true;
            }
            if (block_nums_mult != 1.0) {
                if (need_space) {
                    base_str += string_format("   <color_c_yellow>x %.2f</color>", block_nums_mult);
                }
                else {
                    base_str += string_format("<color_c_yellow>x %.2f</color>", block_nums_mult);
                }
            ...
            if (
                ...
                || item_damage_acid_mult !=1.0|| item_damage_cold_mult != 1.0
                ) {
                ...
                base_str = "    钝击：";
                need_space = false;
                if (item_damage_bash != 0.0) {
                    base_str += string_format("<color_c_yellow>%d</color>", static_cast<int>(item_damage_bash));
                    need_space = true;
                }
                if (item_damage_bash_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", item_damage_bash_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", item_damage_bash_mult);
                    }
                ...
                if (item_damage_cut_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", item_damage_cut_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", item_damage_cut_mult);
                    }
                ...
                base_str = "    酸液：";
                need_space = false;
                if (item_damage_acid != 0.0) {
                    base_str += string_format("<color_c_yellow>%d</color>", static_cast<int>(item_damage_acid));
                    need_space = true;
                }
                if (item_damage_acid_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", item_damage_acid_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", item_damage_acid_mult);
                    }
                ...
                if (item_damage_heat_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", item_damage_heat_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", item_damage_heat_mult);
                    }
                ...
                base_str = "    寒冷：";
                need_space = false;
                if (item_damage_cold != 0.0) {
                    base_str += string_format("<color_c_yellow>%d</color>", static_cast<int>(item_damage_cold));
                    need_space = true;
                }
                if (item_damage_cold_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", item_damage_cold_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", item_damage_cold_mult);
                    }
            ...
            if (armor_bash != 0.0 || armor_cut != 0.0 || armor_stab != 0.0
                ...
                if (armor_bash_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", 1-armor_bash_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", 1-armor_bash_mult);
                    }
                ...
                if (armor_cut_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", 1-armor_cut_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", 1-armor_cut_mult);
                    }
                ...
                if (armor_stab_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", 1-armor_stab_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", 1-armor_stab_mult);
                    }
                ...
                if (armor_bullet_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", 1-armor_bullet_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", 1-armor_bullet_mult);
                    }
                ...
                if (armor_elec_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", 1-armor_elec_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", 1-armor_elec_mult);
                    }
                ...
                if (armor_acid_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", 1-armor_acid_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", 1-armor_acid_mult);
                    }
                ...
                if (armor_heat_mult != 1.0) {
                    if (need_space) {
                        base_str += string_format("   <color_c_yellow>x %.2f</color>", 1-armor_heat_mult);
                    }
                    else {
                        base_str += string_format("<color_c_yellow>x %.2f</color>", 1-armor_heat_mult);
                    }
    ...
    if( is_craft() ) {
        if( unattended_craft_waiting() ) {
            ...
            if( unattended_craft_has_failed() ) {
                status = _( "<bad>无人值守：已经失败，这份半成品已损坏。</bad>" );
            } else if( unattended_craft_is_paused() ) {
                status = string_format( _( "无人值守：<bad>工序已暂停</bad>，还需 <info>%s</info>。\n"
                                            "环境状态：缺少所需工具或环境条件。" ),
                                        to_string( unattended_craft_time_remaining() ) );
            } else if( unattended_craft_is_ready() ) {
                status = _( "<good>无人值守：工序已经完成，可以继续制作。</good>" );
                if( unattended_craft_fail_at() ) {
                    status += string_format( _( " 最迟还可等待 <info>%s</info>。" ),
                                             to_string( unattended_craft_safe_time_remaining() ) );
                }
            } else {
                status = string_format( _( "无人值守：还需 <info>%s</info>。" ),
                                        to_string( unattended_craft_time_remaining() ) );
                if( get_player_character().has_watch() ) {
                    status += string_format( _( " 预计 <info>%s</info> 完成" ),
                                             to_string_time_of_day( unattended_craft_ready_at() ) );
                    if( unattended_craft_fail_at() ) {
                        status += string_format( _( "，最迟 <info>%s</info> 处理" ),
                                                 to_string_time_of_day( *unattended_craft_fail_at() ) );
                    }
                    status += _( "。" );
                }
                if( get_making().unattended_craft().check_environment ) {
                    status += _( "\n环境状态：所需工具与环境条件正常。" );
                }
            }
...
int item::getlight_emit() const
{
    ...
    if (ammo_remaining(nullptr ,true) == 0) {
        return 0;
    }
    if( has_flag( flag_CHARGEDIM ) && is_tool() && !has_flag( flag_USE_UPS ) ) {
        ...
        if (ammo_capacity(loaded_ammo) &&
            ammo_remaining(nullptr,true) < (ammo_capacity(loaded_ammo) / 5)) {
            lumint *= ammo_remaining(nullptr,true) * 5.0 / ammo_capacity(loaded_ammo);
        }
    }
    return lumint;
}
...
bool item::process_cable(map& here, Character* carrier, const tripoint& pos, item* parent_item)
{
    ...
    if (!link->t_veh_safe) {
        if (last_t_abs_pos_is_oob) {
            ...
            if (vp_displayed && vp_displayed->part().has_flag(vehicle_part::carried_flag)) {
                             ...
                             float insulation, const temperature_flag flag, float spoil_modifier )
{
    ...
    if( active && unattended_craft_waiting() ) {
        update_unattended_craft_environment( here, carrier, pos );
    }
...
time_duration item::get_plant_epoch() const
{
    ...
    return type->seed->grow * get_option<float>("宏观种子生长周期比例系数") * calendar::season_ratio() / 3;
}
...
namespace
{
Character *unattended_source_character( const item &craft, Character *carrier,
                                        const tripoint &pos )
{
    if( carrier != nullptr ) {
        return carrier;
    ...
    if( rl_dist( you.pos(), pos ) <= PICKUP_RANGE ) {
        return &you;
    }

    const std::string assigned_crafter = craft.get_var( "crafter", "" );
    if( assigned_crafter.empty() ) {
        return nullptr;
    }
    for( npc &candidate : g->all_npcs() ) {
        if( candidate.name == assigned_crafter &&
            rl_dist( candidate.pos(), pos ) <= PICKUP_RANGE ) {
            return &candidate;
        }
    }
    return nullptr;
...
bool unattended_inventory_has_tool( const inventory &inv, const tool_comp &tool )
{
    if( tool.type == itype_id::NULL_ID() ) {
        return true;
    }
    return tool.count > 0 ? inv.has_charges( tool.type, 1 ) : inv.has_tools( tool.type, 1 );
}

bool unattended_environment_satisfied( const item &craft, map &here, Character *carrier,
                                        const tripoint &pos )
{
    ...
    Character *source = unattended_source_character( craft, carrier, pos );

    inventory map_inv;
    map_inv.form_from_map( pos, PICKUP_RANGE, source, false, false );

    inventory character_inv;
    inventory combined_inv = map_inv;
    if( source != nullptr ) {
        character_inv = source->crafting_inventory( source->pos(), -1, false );
        combined_inv = source->crafting_inventory( pos, PICKUP_RANGE, false );
    }

    for( const std::vector<quality_requirement> &group :
         rec.simple_requirements().get_qualities() ) {
        bool group_satisfied = false;
        for( const quality_requirement &quality : group ) {
            if( quality.has( combined_inv, return_true<item> ) ) {
    ...
    const std::vector<comp_selection<tool_comp>> &selected_tools =
        craft.get_cached_tool_selections();
    if( selected_tools.empty() ) {
        for( const std::vector<tool_comp> &alternatives :
             rec.simple_requirements().get_tools() ) {
            bool group_satisfied = false;
            for( const tool_comp &tool : alternatives ) {
                if( unattended_inventory_has_tool( combined_inv, tool ) ) {
                    group_satisfied = true;
                    break;
                }
            }
    ...
    for( const comp_selection<tool_comp> &selection : selected_tools ) {
        const inventory *source_inv = &combined_inv;
        switch( selection.use_from ) {
            case usage_from::player:
                if( source == nullptr ) {
                    return false;
                }
                source_inv = &character_inv;
                break;
            case usage_from::map:
                source_inv = &map_inv;
                break;
            ...
            case usage_from::none:
                source_inv = &combined_inv;
                break;
            case usage_from::cancel:
            case usage_from::num_usages_from:
                return false;
        }
        if( !unattended_inventory_has_tool( *source_inv, selection.comp ) ) {
            return false;
        }
    }
    return true;
...
{
    return unattended_craft_waiting() && !unattended_craft_is_paused() &&
           calendar::turn >= craft_data_->unattended_ready_at;
}
...
bool item::unattended_craft_is_paused() const
{
    return unattended_craft_waiting() && craft_data_->unattended_paused;
}
...
{
    if( !unattended_craft_waiting() || unattended_craft_is_paused() ||
        craft_data_->unattended_fail_at == calendar::before_time_starts ) {
        return std::nullopt;
    }
...
time_duration item::unattended_craft_time_remaining() const
{
    ...
    const time_point reference = unattended_craft_is_paused() ?
                                 craft_data_->unattended_pause_started_at : calendar::turn;
    return std::max( 0_turns, craft_data_->unattended_ready_at - reference );
}
...
time_duration item::unattended_craft_safe_time_remaining() const
{
    ...
    const time_point reference = unattended_craft_is_paused() ?
                                 craft_data_->unattended_pause_started_at : calendar::turn;
    return std::max( 0_turns, craft_data_->unattended_fail_at - reference );
}
...
void item::start_unattended_craft()
{
    ...
    craft_data_->unattended_paused = false;
    craft_data_->unattended_pause_started_at = calendar::before_time_starts;
    ...
    craft_data_->unattended_env_check_at = data.check_environment ?
                                           std::min( calendar::turn + 1_minutes,
                                                   craft_data_->unattended_ready_at ) :
                                           calendar::before_time_starts;
    active = data.check_environment;
}
...
void item::finish_unattended_craft()
{
    ...
    craft_data_->unattended_paused = false;
    craft_data_->unattended_pause_started_at = calendar::before_time_starts;
    craft_data_->unattended_env_check_at = calendar::before_time_starts;
    active = false;
}

void item::update_unattended_craft_environment( map &here, Character *carrier,
        const tripoint &pos )
{
    if( !unattended_craft_waiting() || !get_making().unattended_craft().check_environment ) {
        return;
    }
    if( craft_data_->unattended_env_check_at != calendar::before_time_starts &&
        calendar::turn < craft_data_->unattended_env_check_at ) {
        return;
    }

    const time_point scheduled_check = craft_data_->unattended_env_check_at ==
                                       calendar::before_time_starts ? calendar::turn :
                                       craft_data_->unattended_env_check_at;
    const bool environment_ok = unattended_environment_satisfied( *this, here, carrier, pos );

    if( !environment_ok ) {
        if( !craft_data_->unattended_paused ) {
            craft_data_->unattended_paused = true;
            craft_data_->unattended_pause_started_at = std::min( calendar::turn, scheduled_check );
            if( carrier != nullptr ) {
                carrier->add_msg_if_player( m_warning,
                                            _( "缺少所需工具或环境条件，无人值守工序已经暂停。" ) );
            } else {
                add_msg_if_player_sees( pos, m_warning,
                                        _( "%s的无人值守工序因为条件中断而暂停。" ), tname() );
            }
        }
        craft_data_->unattended_env_check_at = calendar::turn + 1_minutes;
        active = true;
        return;
    }

    if( craft_data_->unattended_paused ) {
        const time_duration paused_for = calendar::turn -
                                         craft_data_->unattended_pause_started_at;
        craft_data_->unattended_ready_at += paused_for;
        if( craft_data_->unattended_fail_at != calendar::before_time_starts ) {
            craft_data_->unattended_fail_at += paused_for;
        }
        craft_data_->unattended_paused = false;
        craft_data_->unattended_pause_started_at = calendar::before_time_starts;
        if( carrier != nullptr ) {
            carrier->add_msg_if_player( m_good,
                                        _( "所需工具与环境已经恢复，无人值守工序继续进行。" ) );
        } else {
            add_msg_if_player_sees( pos, m_good,
                                    _( "%s的无人值守工序恢复进行。" ), tname() );
        }
    }

    if( calendar::turn >= craft_data_->unattended_ready_at ) {
        craft_data_->unattended_env_check_at = calendar::before_time_starts;
        active = false;
    } else {
        craft_data_->unattended_env_check_at = std::min( calendar::turn + 1_minutes,
                                                craft_data_->unattended_ready_at );
        active = true;
    }
}
